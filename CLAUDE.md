...
**My fork is `jsoubrier/ai_agent_workshop`.**

Anything that **creates** — `gh issue create`, `gh pr create` — passes
`--repo jsoubrier/ai_agent_workshop`. Never write to
...
`qhgenomics` repos are off limits entirely — no reads, no writes, no `gh` calls
against them.
...
**`mytools` is written in Go.** Every subcommand and every test. Don't
...
- Exit codes: `0` success, `1` every error — bad input data and usage errors alike.
  `2` is never returned. Real bedtools exits `1` for usage errors too and the oracle
  wins, so the older `0`/`1`/`2` scheme this file used to mandate is gone; see
  `SPEC.md` §5 and §9.
